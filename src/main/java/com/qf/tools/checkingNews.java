...
public class checkingNews {
    ...
    public static boolean checkingNews(String np_content,String newContent) {

//   比较目标文章和对比文章的长度，并对strA和strB进行赋值，strA长度必须长于strB
        String strA = null;
        String strB = null;
        if (np_content.length() > newContent.length()) {
            strA = np_content;
            strB = newContent;
        } else {
            strA = newContent;
...
//        返回两篇文章的相似度，以50%的相似度为判断它们是否重复
        return SimilarDegree(strA, strB)>0.5;
    }
    /*
     * 计算相似度
     * */
